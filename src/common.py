...
def parse_tweet_info(data):
    ...
    tweet = {
        ...
        "date": parse_time(data['created_at']),
        "timestamp":int(datetime.timestamp(datetime.strptime(parse_time(data['created_at']), '%Y-%m-%d %H:%M:%S'))),
...
def parse_long_tweet(response,item,headers):
    """
    解析长推文
    """
